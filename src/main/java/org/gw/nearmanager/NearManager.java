...
public class NearManager extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (getServer().getPluginManager().getPlugin("LuckPerms") == null) {
            getLogger().severe(HexColors.colorize("&cLuckPerms не найден! Этот плагин требуется для работы NearManager", false, getLogger()));
            ...
            for (net.luckperms.api.node.Node node : user.getNodes()) {
                ...
                if (perm.startsWith("nearmanager.near-radius.")) {
                    try {
                        ...
                        newRadius = radius; // Берём последнее валидное разрешение
                    } catch (NumberFormatException ignored) {
                    }
                }
            }
    ...
    public void removePlayerRadius(UUID playerUUID) {
        playerRadiusCache.remove(playerUUID);
    }
